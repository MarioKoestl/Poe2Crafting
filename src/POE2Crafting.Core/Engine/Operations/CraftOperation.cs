...
/// <summary>
/// One currency operation (CurrencyDef.Op). The engine runs the generic checks (rarity, target class, mirrored, corrupted ...)
/// before calling <see cref="Check"/>; <see cref="Preview"/> and <see cref="Execute"/> are only called when the check passed.
/// </summary>
public abstract class CraftOperation
{
    ...
    /// <summary>Class group used when the currency data defines none (GameData target groups).</summary>
    ...
    public virtual bool AcceptsOmen(CraftContext ctx, OmenDef omen) => CraftingEngine.OpOfOmenTarget(omen.TargetCurrency) == Op;
